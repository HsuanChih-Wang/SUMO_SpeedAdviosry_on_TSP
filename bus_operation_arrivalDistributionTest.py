...
def run():
    ...
    def recordVehArrivalTime(vehX, indicator):

        def writeToCSV(vehInfo, vehID):
            from datetime import datetime
            now = datetime.now()
            with open('BusArrivalTime_vc0.5.csv', 'a', newline='') as f:
                writer = csv.writer(f)
                list = [vehID, vehInfo['startTime'], vehInfo['arrivalTime']]
                writer.writerow(list)

        currentTime = traci.simulation.getTime()
        vehID = vehX['vehID']
        if (indicator == 0):
            vehInfoList.update({vehID: {'startTime': currentTime, 'arrivalTime': 0}})

        elif indicator == 1:
            try:
                vehInfoList[vehID]['arrivalTime'] = currentTime
                writeToCSV(vehInfo=vehInfoList[vehID], vehID=vehID)
                del vehInfoList[vehID]
            except:
                print("vehID不存在")
